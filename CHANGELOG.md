...
The package always talked to a subprocess or an HTTP service; nothing in the transport, the
allow-list, the HMAC callbacks or the replay guard was Python-specific. The name was.

- **Namespace** `Simtabi\\Laranail\\Python` → `Simtabi\\Laranail\\Polyglot`; facade `Python` →
  `Polyglot`; config `laranail.python.*` → `laranail.polyglot.*`; env `PYTHON_*` → `POLYGLOT_*`.

- **`ResolvedScript::$interpreter` (a `string`) became `ResolvedCommand::$prefix` (a
  `list<string>`).** The resolver insisted the value be an absolute path that `is_file()` and
  `is_executable()` — right for `/usr/bin/python3`, and incapable of expressing
  `['docker','run','--rm',$image]` or `[]` for a compiled binary that runs itself. A single path
  keeps every old check; a longer list is not path-checked, because `docker` is meant to come from
  `$PATH` and the words after it are arguments rather than files. The command is still built as an
  array and never passed through a shell.

- **`process.interpreters` → `process.runtimes`**, and a script entry's `interpreter` key is now
  `runtime`.

- **`fastapi()` and `flask()` are gone from `HttpClient`.** They were the only place the *interface*
...
- **`LARANAIL_POLYGLOT_BIN` was always empty in CI.** The `actions/setup-python`
...
- **HTTP transport.** Named services resolved from config — base URL, timeout,
  ...
  `Polyglot::service('name')` hands back a real `PendingRequest`, so `attach()`,
...
- **`Polyglot::fake()`** with `assertSent()`, `assertSentTo()`,
  `assertSentTimes()`, `assertNothingSent()`. A fake reports itself as faked
  rather than healthy.
- **Commands** `laranail::polyglot.{doctor,health,run,install,make-service}`.
