 ...
 * the health contract — so adding one is configuration rather than code, and
 * the service could be written in anything.
 *
 * The contract used to carry `fastapi()` and `flask()` beside `service()`.
 * They were the only place the *interface* named a language, which made a
 * package that talks to any runtime look like one that talks to Python and
 * tolerates the rest. `service('fastapi')` says the same thing and does not
 * privilege one framework in the type.
 */
final readonly class HttpClientService implements HttpClient
{
    ...
    private function applyTls(PendingRequest $request, ServiceDefinition $definition): PendingRequest
    {
        ...
        $this->logger->warning('Service CA certificate configured but not found; using the system CA bundle', [
