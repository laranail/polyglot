...
|---|---|---|---|
| Narrow | `Contracts\TransportContract` | `CallResult` | you want the call, not the mechanism |
| Wide — HTTP | `Contracts\HttpClient` | `PendingRequest` | you want `attach()`, `sink()`, streaming |
| Wide — process | `Contracts\ProcessRunner` | `CallResult` + output callback | you want incremental stdout or per-call env |
...
`Polyglot::run()` returns a `CallResult` and a non-2xx is a result, not a throw.
...
make. `laranail::polyglot.doctor` reports it as disabled rather than staying
