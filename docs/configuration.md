...
Which service `Polyglot::service()` uses when no name is given.
...
| `verify_ssl` | `true` | `false` is insecure and `laranail::polyglot.doctor` reports it as such. |
...
`fastapi` and `flask` ship pre-defined with `POLYGLOT_FASTAPI_*` and `POLYGLOT_FLASK_*` env names. They
...
| `process.allow_path_lookup` | `false` | `false` means interpreters are absolute paths from `process.runtimes`, so `$PATH` cannot substitute one. |
...
| `process.runtimes` | — | Named absolute paths. |
...
`config('laranail.toolkit.python.services.*')` becomes `config('laranail.polyglot.services.*')`. **Env
