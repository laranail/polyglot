...
| A caller reaches `../../.env` or `/tmp/evil.py` | **Scripts are named, not pathed.** `Polyglot::run('embed')` looks `embed` up in `laranail.polyglot.process.scripts`. A caller cannot express a path at all, so there is nothing to sanitise. |
...
| An error message tells an attacker which knob to turn | Every failure is a bare `401` with no detail. The reason travels on the `CallbackRejected` event, where your application can alert on it. |
...
`php artisan laranail::polyglot.doctor` reports, and exits non-zero on, each of:
