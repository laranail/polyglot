...
`php artisan laranail::polyglot.make-service` generates a service that already
...
failed goes out on the `CallbackRejected` event instead, because naming it
...
Event::listen(function (CallbackReceived $event): void {
...
```php
'secrets' => [env('POLYGLOT_CALLBACK_SECRET_NEW'), env('POLYGLOT_CALLBACK_SECRET')],
```
