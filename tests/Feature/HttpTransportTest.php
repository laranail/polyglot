...
final class HttpTransportTest extends TestCase
{
    ...
    {
        config()->set('laranail.polyglot.services.bare', ['timeout' => 5]);
    ...
    public function test_the_health_contract_is_configurable(): void
    {
        ...
        config()->set('laranail.polyglot.services.fastapi.health_key', 'state');
        config()->set('laranail.polyglot.services.fastapi.healthy_value', 'up');
    ...
    public function test_a_non_string_health_value_still_matches(): void
    {
        ...
        config()->set('laranail.polyglot.services.fastapi.healthy_value', '1');
    ...
    {
        config()->set('laranail.polyglot.services.fastapi.auth', ['scheme' => 'bearer']);
    ...
    public function test_a_successful_call_carries_the_decoded_body(): void
    {
        ...
        $result = Polyglot::run('fastapi:embed', ['text' => 'hi']);
    ...
    {
        $result = Polyglot::run('not-a-service:x');
    ...
    {
        config()->set('laranail.polyglot.services.fastapi.verify_ssl', false);
        self::assertStringContainsString('insecure', $this->client()->definition('fastapi')->tlsMode());

        config()->set('laranail.polyglot.services.fastapi.verify_ssl', true);
    ...
    {
        config()->set('laranail.polyglot.services.fastapi.base_url', 'https://user:pass@svc.internal');
    ...
    public function test_a_server_error_is_retried(): void
    {
        ...
        config()->set('laranail.polyglot.services.fastapi.retry_times', 3);
        config()->set('laranail.polyglot.services.fastapi.retry_sleep_ms', 1);

        $result = Polyglot::run('fastapi:predict');
    ...
    {
        config()->set('laranail.polyglot.services.fastapi.base_url', 'file:///etc/passwd');
